...
(defclass discord-base ()
  ((%websocket-client :accessor websocket-client)
   ...
   (%chanic :accessor res-thread :initarg :res-thread)
  ...
  (let ((client (wsd:make-client
    ...
    (wsd:on :open client
            (lambda ()
              ...
              (setf (seq discord) 0)
              (unless (and (slot-boundp discord '%chanic) (bt:thread-alive-p (res-thread discord)))
                (setf (res-thread discord)
                      (bt:make-thread
                       (lambda () (loop (lparallel:receive-result *chan*)))
                       :name "chanic")))))
...
(defvar *werk* (lparallel:make-kernel 200 :name "harmonious"))
(setf lparallel:*kernel* *werk*)
(defvar *chan* (lparallel:make-channel))

(defgeneric dispatch (discord event data))
(defmethod dispatch ((discord discord-base) event data)
  (let ((lparallel:*kernel* *werk*))
       ...
       (let ((read-thread (slot-value (websocket-client discord)
         ...
         (if read-thread
             (progn (bt:destroy-thread read-thread)
                    (l:logm :important "Killed read thread using hack 1."))
             (l:logm :important "Read thread not found using hack 1.")))
       (map nil (lambda (x)
                  (bt:destroy-thread x)
                  (l:logm :important "Read thread killed using hack 2."))
...
(defun foo (bar)
  ...
  (flet ((baz (x)
           (when (evenp x) (return-from foo x))))
    (declare (dynamic-extent (function baz)))
    (map nil #'baz bar)))
